...
pipeline {
    ...
    environment {
        DOCKER_IMAGE = 'raina1994/mycoursework'
    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                ...
                sh 'kubectl rollout restart deployment/mycoursework'

                sh 'kubectl rollout status deployment/mycoursework --timeout=120s'

                echo '---------- Deployment Status ----------'
                sh 'kubectl get pods -l app=mycoursework'
                sh 'kubectl get services mycoursework-service'
            }
        }
    }
